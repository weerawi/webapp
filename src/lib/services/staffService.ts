...
import { AppDispatch } from "../store/store";
import { fetchSupervisorsFromFirestore } from "./supervisorService";
import { fetchHelpersFromFirestore } from "./helperService";
import { mockAttendanceData } from "../data/mockAttendanceData";
import { setAttendance, AttendanceRecord } from "../store/slices/attendanceSlice";

// Combined fetch for both supervisors and helpers
export const fetchAllStaffFromFirestore = async (dispatch: AppDispatch) => {
  await Promise.all([
    fetchSupervisorsFromFirestore(dispatch),
    fetchHelpersFromFirestore(dispatch)
  ]);
};

// Keep attendance functions as they are
export const fetchAttendanceFromFirestore = async (dispatch: AppDispatch) => {
  await new Promise(resolve => setTimeout(resolve, 500));
  const attendanceList = mockAttendanceData.attendance as AttendanceRecord[];
  dispatch(setAttendance(attendanceList));
...
export const updateAttendanceAndSync = async (dispatch: AppDispatch) => {
  const raw = Array.isArray(mockAttendanceData) ? mockAttendanceData : [];
  const normalized: AttendanceRecord[] = raw.map((r: any, i: number) => ({
    id: r.id || `mock-${i}`,
    staffId: r.staffId || `STAFF-${i}`,
    staffName: r.staffName || "Unknown",
    area: r.area || "Unknown",
    teamNumber: r.teamNumber ?? 0,
    role: r.role || "helper",
    date: r.date || new Date().toISOString().slice(0, 10),
    timeIn: r.timeIn || "",
    timeOut: r.timeOut || "",
    status: r.status === "in" ? "in" : "out",
    gpsLocationIn: r.gpsLocationIn || "",
    gpsLocationOut: r.gpsLocationOut || "",
    imageUrl: r.imageUrl,
  }));
  dispatch(setAttendance(normalized));
};