...
export function SupervisorWiseReport() {
  ...
  const calculateSupervisorWiseData = () => {
    ...
    // Get previous date records for previous day balance calculation
    ...
    // Calculate previous day balance by team
    const previousDayBalanceByTeam = new Map<string, number>();
    previousDateRecords.forEach((record) => {
      const key = `${record.teamNo}-${record.supervisor}-${record.area}`;
      const count = previousDayBalanceByTeam.get(key) || 0;
      previousDayBalanceByTeam.set(key, count + 1);
    });
    ...
    filteredRecords.forEach((record) => {
      ...
      if (!teamMap.has(key)) {
        const previousDayBalance = previousDayBalanceByTeam.get(key) || 0;
        const returned = 0; // This should be calculated based on your business logic
        const broughtForward = previousDayBalance - returned;
    ...
    const finalData = Array.from(teamMap.values()).map((data) => {
      ...
      const teamDates = new Set(
        filteredRecords
          .filter(
            (r) =>
              r.teamNo === data.teamNo &&
              r.supervisor === data.supervisor &&
              r.area === data.area
          )
          .map((r) => r.date)
      );
      ...
      // Calculate unable to attend (this is what wasn't completed)
      data.unableToAttend = data.totalJobs - data.ocDone - data.rcDone;
  ...
  const calculateAreaTotals = (areaData: SupervisorWiseData[]) => {
    return areaData.reduce(
      (totals, data) => ({
        previousDayBalance: totals.previousDayBalance + data.previousDayBalance,
        ...
        alreadyDisconnected:
          totals.alreadyDisconnected + data.alreadyDisconnected,
    ...
    <Card className="gap-1 py-2 overflow-x-hidden">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>
            Hagra Holdings Lanka (Pvt) Ltd - Daily DC/RC Summary
          </CardTitle>
          ...
          <Table className="w-full">
            <TableHeader className="sticky top-0 bg-background z-20 shadow-sm">
              <TableRow>
                ...
                <TableHead colSpan={10} className="text-center border">
                  Reasons
                </TableHead>
              </TableRow>
              <TableRow>
                ...
                return (
                  <React.Fragment key={area}>
                    ...
                    <TableRow className="font-bold bg-gray-100">
                      ...
                      <TableCell className="text-center">
                        {areaTotals.previousDayBalance}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.returned}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.broughtForward}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.jobsReceived}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.totalJobs}
                      </TableCell>
                      ...
                      <TableCell className="text-center">
                        {areaTotals.unSolvedCusComp}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.gateClosed}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.meterRemoved}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.alreadyDisconnected}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.wrongMeter}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.billingError}
                      </TableCell>
                      <TableCell className="text-center">
                        {areaTotals.cantFind}
                      </TableCell>
              ...
              {(() => {
                const grandTotal = Object.values(groupedData).reduce(
                  (grand, areaData) => {
                    ...
                    return {
                      previousDayBalance:
                        grand.previousDayBalance +
                        areaTotals.previousDayBalance,
                      returned: grand.returned + areaTotals.returned,
                      broughtForward:
                        grand.broughtForward + areaTotals.broughtForward,
                      jobsReceived:
                        grand.jobsReceived + areaTotals.jobsReceived,
                      ...
                      unSolvedCusComp:
                        grand.unSolvedCusComp + areaTotals.unSolvedCusComp,
                      gateClosed: grand.gateClosed + areaTotals.gateClosed,
                      meterRemoved:
                        grand.meterRemoved + areaTotals.meterRemoved,
                      alreadyDisconnected:
                        grand.alreadyDisconnected +
                        areaTotals.alreadyDisconnected,
                ...
                return (
                  <TableRow className="font-bold bg-gray-200 border-t-2">
                    ...
                    <TableCell className="text-center">
                      {grandTotal.previousDayBalance}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.returned}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.broughtForward}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.jobsReceived}
                    </TableCell>
                    <TableCell className="text-center">
                    ...
                    <TableCell className="text-center">
                      {grandTotal.unSolvedCusComp}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.gateClosed}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.meterRemoved}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.alreadyDisconnected}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.wrongMeter}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.billingError}
                    </TableCell>
                    <TableCell className="text-center">
                      {grandTotal.cantFind}
                    </TableCell>
